...
def write_to_cell(file_path: str, cell_id: int, content: str) -> str:
    """
    Overwrite the content of an existing cell at a given cell index. Specify content to either python code or markdown code.
    """
    try:
        with open(file_path, "r", encoding="utf-8") as f:
            notebook = json.load(f)
...
def read_cell(file_path: str, cell_id: int) -> str:
    """
    Read the full content of a cell at a given integer cell index. This includes its type, 
    execution count, metadata, source content, and output if the cell is a code cell.
    """
    try:
        with open(file_path, "r", encoding="utf-8") as f:
            notebook = json.load(f)
...
def read_notebook(file_path: str) -> str:
    """
    Retrieve all cells from a notebook. This includes all of the cells and their order 
    in the notebook, their content, output, cell type, execution count, and other metadata.
    """
    try:
        with open(file_path, "r", encoding="utf-8") as f:
            notebook = json.load(f)
